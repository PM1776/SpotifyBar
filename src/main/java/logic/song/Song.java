...
public class Song {
	...
	/** The duration of the song in seconds as an int. */
	private int duration;
	
	/** The progress into the song in seconds as an int. */
	private int progress;
	
	/** The last time in Unix milliseconds the song data was updated. */
	 ...
	 * @see JsonPath
	 * */
	private static Map<String, String> jsonPathsMap = new HashMap<>();
	
	static {
		/*
		 *  Creates a Map to get keys from returned JSON's using a JSONPath expression, 
		 *  which parses the JSON to get the particular value.
		 */
		Map<String, String> jsonPaths = new HashMap<>();
		jsonPaths.put("name", ".name");
		jsonPaths.put("artists", ".artists[*]");
		jsonPaths.put("albumName", ".album.name");
		jsonPaths.put("albumURL", ".album.images[0].url");
		jsonPaths.put("previewURL", ".preview_url");
		jsonPaths.put("ID", ".id");
		jsonPaths.put("duration", ".duration_ms");
		jsonPaths.put("progress", "$.progress_ms");
		jsonPaths.put("timestamp", "$.timestamp");
		jsonPaths.put("playing", "$.is_playing");
		jsonPaths.put("contextURI", "$.context.uri");
		jsonPaths.put("albumURI", ".album.uri");
		jsonPaths.put("trackNumber", ".track_number");
	...
	public Song() {
		this.setArtists(""); // prevents nulls with LinkedHashSet artists
	}
	
	/** 
	 * Constructor for creating blank songs to display a message (name field) instead. 
	 * The artists are
	 * @param message The message to display, attributed to the name field.
	 */
	 ...
	 * @param albumURL The URL to get the album cover.
	 * @param albumURI The URI to play the album.
	 ...
	 * @param progress The progress in the track in seconds.
	 * @param timeStamp The time in UNIX milliseconds the track data was updated.
	 * @param playing A boolean of if the track is playing on Spotify.
	 * @param contextURI The URI of the album or playlist to continue playing from when 
	 * 		the track finishes.
	...
	public static HashMap<String, String> getJsonPathsMap() {
		return new HashMap<String, String>(jsonPathsMap);
	}
	
	public static void setJsonPaths (Map<String, String> jsonPaths) {
		jsonPathsMap.putAll(jsonPaths);
	}
	...
	public Song getDeepCopy () {
		...
		Song copy = new Song(this.getName(), this.getArtists(), this.getAlbumName(), 
				...
				this.getProgress(), this.getTimestamp(), this.isPlaying(),
		...
		if ( !(o instanceof Song) ) {
			...
			for (Field field : this.getClass().getDeclaredFields()) {
				...
				try {
					...
					final String getOrIs = (!field.getType().isPrimitive()) ? "get" : "is";
		    		
					Method method = this.getClass().getDeclaredMethod(
							getOrIs + Character.toUpperCase(field.getName().charAt(0))
	 ...
	 */
	public static Song[] initializeFromJSON (String json) {
			...
			for (String field : fields) {
				try {
					if (field != "artists") {
						...
						String jsonPath = (jsonPathsMap.get(field).charAt(0) == '$') ?
								jsonPathsMap.get(field) : 
									baseJSONPath.concat(jsonPathsMap.get(field));
	...
	public static Song[] initializeFromJSON (String json, JSONPreset preset) {
		return initializeFromJSON(json, preset.getBaseJSONPath(), 
				preset.getJsonPathKeys());
	}
}
