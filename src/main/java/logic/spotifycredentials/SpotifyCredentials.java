...
public class SpotifyCredentials  {
    ...
    private static final int CODE_AUTHORIZATION = 0;
    ...
    /** Is true if authorization completes successfully. */
    private boolean authSuccessful;
    
    /** 
	 * Empty constructor that gets no authorization to the Spotify Web API.
	 */
	 ...
	 * to make requests.
	 */
    public SpotifyCredentials (boolean authCodeFlow) {
     ...
     * @throws UnknownHostException Thrown when unable to connect to the Internet.
	 */
    public SpotifyCredentials (boolean authCodeFlow, Runnable init) {
    	this.authCodeFlow = authCodeFlow;
    	if (authCodeFlow) {
    		getAuthByPKCE (init);
    	} else {
	...
	private void getAuthByPKCE (Runnable init) { 
		...
		String[] paramsToReturn = {"code"};
		
		ap = new ApprovalBrowser(endPoint + query, redirectURI, paramsToReturn);
		
		ap.addRedirectURIListener(() -> {
			requestAccessToken(CODE_AUTHORIZATION);
		});
		
		// user closed ap without granting access
		...
		// redirectURI if approved and call requestAccessToken() for auth
	...
	public String getRefreshToken() {
		if (authCodeFlow) {
			requestAccessToken(CODE_REFRESH);
	...
	private void requestAccessToken (int flow) {
        ...
        String grantType = (flow == CODE_AUTHORIZATION) ? "authorization_code" : 
        ...
        switch (flow) {
	        ...
	        // refresh for Authorization Code + PKCE
        	case CODE_REFRESH:
        		try {
					response = Unirest.post("https://accounts.spotify.com/api/token")
							.header("Authorization", authorization)
	...
	/**
	 * Returns a boolean of if authorization was successful.
	 * 
	 * @return A boolean which is <i>true</i> if successful, or <i>false</i> if 
	 * uncompleted or unsuccessful.
	 */
