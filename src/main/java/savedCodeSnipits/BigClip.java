 ...
 * </ul>
 * N.B. Remove  notation and logging to use in 1.3+
 * 
 * @since 1.5
 * @version 2009-08-17
...
public class BigClip implements Clip, LineListener {
	 ...
	 * This convenience constructor allows us to obtain a SourceDataLine for the
	 * BigClip that uses the same AudioFormat as the original Clip.
	 * 
	 ...
	 * dialog. If null, there will be no progress shown.
	 */
	...
	public void start() {
		Runnable r = new Runnable() {
			public void run() {
				try {
					/*
					 * Should these open()/close() calls be here, or explicitly called by user
					 * program? The JavaDocs for line suggest that Clip should throw an
					 * IllegalArgumentException, so we'll stick with that and call it explicitly.
					 */
					dataLine.open();
					...
					while ((bytesRead = inputStream.read(data, 0, data.length)) != -1
							&& (loopCount == Clip.LOOP_CONTINUOUSLY || countDown > 0) && active) {
						logger.log(Level.FINEST, "BigClip.start() loop " + framePosition);
						...
						if (format.getChannels() < 2) {
							...
							// With javafx imported, Arrays import wouldn't work
						...
						if (framePosition >= loopPointEnd) {
							...
							inputStream.reset();
							countDown--;
							logger.log(Level.FINEST, "Loop Count: " + countDown);
						}
						...
						if (startOrMove) {
							data = new byte[bufSize / bufferUpdateFactor];
							startOrMove = false;
						}
					}
					logger.log(Level.FINEST, "BigClip.start() loop ENDED" + framePosition);
					active = false;
					dataLine.drain();
					dataLine.stop();
					/*
					...
					logger.log(Level.SEVERE, "No sound line available!", lue);
					if (parent != null) {
						JOptionPane.showMessageDialog(parent, "Clear the sound lines to proceed",
								"No audio lines available!", JOptionPane.ERROR_MESSAGE);
					}
				}
			}
		};
	 ...
	 * clip. This can be handy for determining a fraction to scal visual
	 * representations.
	 * 
